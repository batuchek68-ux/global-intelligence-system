...
DIST = ROOT / "dist"
PACKAGE = DIST / "international-trade-ai.zip"
...
def build_release() -> dict:
    ...
    manifest = {
        ...
        "upload_steps": [
            ...
            "Upload or unzip international-trade-ai.zip into the repository root.",
            ...
            "For one-command cloud testing, set GITHUB_TOKEN and GITHUB_REPOSITORY, then run python workflows/cloud_run.py --create-repo --confirm-upload.",
            "For an interactive setup, run .\\setup-cloud-test.ps1 and enter the repository plus token when prompted.",
            "For a stable Windows launcher, run .\\run-cloud-test.cmd -CreateRepo after setting GITHUB_TOKEN and GITHUB_REPOSITORY.",
            "On Windows, run .\\运行云端测试.ps1 -CreateRepo after setting GITHUB_TOKEN and GITHUB_REPOSITORY.",
